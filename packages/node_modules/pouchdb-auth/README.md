...
And add the new password_scheme 'openpgp' supports.

## OpenPGP Password Scheme

It uses the public key auth instead of password auth.
The server private key is stored in the config section: `pouchdb_server/private_key`.
The format should be a PGP Armed or base32768.
The user's public key is stored the attribute `password` of the user document.

add a new API to get the system's Public Key. `_sys/pk`

SignUp:

user send encrypted data using server's public key:

```js
  "name": "login name",
  "password": "user's public key is PGP Armed",
  "time": "datetime"
```

Server response the signature of the data if successful.
...
```js
{
  "name": "loginname verified"
  "uuid": "verified the server uuid"
  "device": "optional"
  "time": "expired datetime if exists"
}
```
这个就不改了。只修改login. pk也不加了，就用password.
return token: "username+device:expiredTime(失效的时间):sign(username+设备id+expiredTime+userPK)"

login的参数username, password,username保持不变，password变为一个带签名的Message,内容为上述:

通过设置时间(time)决定该password消息到期的时间。
