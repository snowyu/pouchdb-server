...
exports.multiUserLogIn = function (username, password, callback) {
  ...
  return utils.nodify(getUserDoc(db, username).then(function (doc) {
    userDoc = doc;
    if (doc.password_scheme === 'openpgp') {
      password = openpgp.message.readArmored(password);
      // get system private key to decrypt password data
      // return privateKey(exports.config).then(function(aKey){
        return openpgp.decrypt({
          message: password,
          publicKeys: openpgp.key.readArmored(doc.password).keys,
          privateKeys: privateKey.keys
        }).then(function(text){
          var vData = JSON.parse(text.data);
          if (vData.time) vData.time = new Date(vData.time);
          var result = vData.name === doc.name &&
            (!vData.time || vData.time >= new Date()) &&
            Array.isArray(text.signatures) &&
            text.signatures.length &&
            text.signatures[0].valid;
          if (result && exports.config) {
            var uuid = exports.config.get('couchdb', 'uuid');
            if (uuid) result = vData.uuid === uuid;
          }
          console.log(result);
          return result;
        })
      ...
      .then(function (derived_key) {
        return derived_key === userDoc.derived_key;
      });
    }
  }).then(function (ok) {
    if (!ok) {
